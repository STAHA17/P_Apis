...
class GenaticController extends Controller
{
    ...
    private function runGeneticAlgorithm($power, $durations,$a_ids,$u_data)
    {
        ...
        global $user_Data,$appliancePower,$applianceDuration, $totalAppliancePower, $appliancePowerBy15Min,$totalAppliancePowerBy15Min,$applianceIds,$firstEntryDate;
    ...
    private function printSchedule($schedule, $appliancePower)
    {
        ...
        for ($slot = 0; $slot < $timeSlots; $slot++) {
                if ($schedule[$slot] != 0) {
                        $sc['start_time'] = 'ljkjl'; // Replace with your actual start time logic
                        $sc['end_time'] =  'ljkjl';   // Replace with your actual end time logic
                        $sc['date'] =  $firstEntryDate;
                        $sc['user_id'] =  $user_Data['user_id'];
                        $sc['appliance_id'] =  $applianceIds[$schedule[$slot] - 1];        
                    // Call the store method and pass the request data
                        $sc->save();
        ...
        //     if ($schedule[$slot] != 0) {
        //         $data = [
        //             'start_time' => 'ljkjl', // Replace with your actual start time logic
        //             'end_time' => 'ljkjl',   // Replace with your actual end time logic
        //             'date' => $firstEntryDate,
        //             'user_id' => $user_Data['user_id'],
        //             'appliance_id' => $applianceIds[$schedule[$slot] - 1],
        //         ];
    
        //         // Call the store method and pass the request data
        //         $request = new Request($data);
        //         $response = $sc->store($request);
        ...
        //             echo "Error saving schedule for time slot $slot";
        //         }
        //         echo "Time Slot $slot: ";
        //         echo "Appliance " . $applianceIds[$schedule[$slot]-1];
        //     }
